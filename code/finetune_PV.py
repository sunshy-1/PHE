...
parser.add_argument('--dif_model_dir', type=str, default='finetune_models',
                    help='The address for storing the models and optimization results.')
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    with torch.no_grad():
        ...
        valid_ndcg = np.average([ndcg_at_k(resi, this_k) for resi in valid_res])
        valid_mrr  = np.average(mean_reciprocal_rank(valid_res))
        valid_recall = np.average([recall_at_k(resi, this_k) for resi in valid_res])
        ...
        st = time.time()
        print(("cuda: %d Epoch: %d (%.1fs)  LR: %.5f Train Loss: %.2f  Valid Loss: %.2f  Valid NDCG: %.4f Valid MRR: %.4f Valid RECALL: %.4f") % \
              (args.cuda, epoch, (st-et), optimizer.param_groups[0]['lr'], np.average(train_losses), \
                    loss.cpu().detach().tolist(), valid_ndcg, valid_mrr, valid_recall))
        stats += [[np.average(train_losses), loss.cpu().detach().tolist()]]
        eval_loss_list.append(loss.item())
        eval_NDCG_list.append(valid_ndcg)
        del res, loss
...
with torch.no_grad():
    ...
    test_ndcg = [ndcg_at_k(resi, this_k) for resi in test_res]
    print('Best Test NDCG: %.4f' % np.average(test_ndcg))
    test_mrr = mean_reciprocal_rank(test_res)
    print('Best Test MRR:  %.4f' % np.average(test_mrr))
    test_recall = [recall_at_k(resi, this_k) for resi in test_res]
    print('Test RECALL: %.4f' % np.average(test_recall))

with open("PV.txt","a") as f:
    f.write(str( np.average(test_ndcg)))
    f.write(' ')
    f.write(str(np.average(test_mrr)))
    f.write(' ')
    f.write(str(np.average(test_recall)))
    f.write('\n')
